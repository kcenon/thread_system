 ...
 * and a name. It inherits from std::enable_shared_from_this to allow creating shared_ptr from this.
 *
...
public:
	 ...
	 */
	priority_job(const std::function<std::tuple<bool, std::optional<std::string>>(void)>& callback,
...
public:
	 ...
	 * @return std::shared_ptr<priority_job_queue<priority_type>> A shared pointer to the associated
	 * priority job queue.
	 */
