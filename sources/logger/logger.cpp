...
auto logger::write(
	...
	std::optional<std::chrono::time_point<std::chrono::high_resolution_clock>> start_time) -> void
{
...
auto logger::do_work() -> void
{
	...
	while (!remaining_logs.empty())
	{
		...
		std::string log = (worked) ? current_log->log()
...
auto logger::write_to_file(const std::string& message) -> void
{
	...
	const std::string backup_name = std::format("{}_{:%Y-%m-%d}.backup", title_, year_month_day);
