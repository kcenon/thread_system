...
public:
	 ...
	 * @return std::tuple<bool, std::optional<std::string>> A tuple containing:
	 *         - bool: Indicates whether the stop operation was successful (true) or not (false).
	 *         - std::optional<std::string>: An optional string message, typically used for error
	 * descriptions.
	 */
...
private:
	...
	std::condition_variable worker_condition_;	  ///< Condition variable for thread synchronization
